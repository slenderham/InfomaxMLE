...
class Readout:
	def __init__(self, inputDim, recDim, outDim, random=False):
		...
		if random:
			...
			self.beta = 1;
			self.tau_v = np.clip(0.6 + np.random.randn(self.recDim, 1)*0.05, 0.1, 1);
		else:
			self.IH, self.HH, self.beta, self.tau_v = pickle.load(open("trained_net.txt", "rb"))

		self.rHH = self.rIH = self.rHO = 4e-3
		...
		# readout weights
		self.HO = np.random.randn(outDim, recDim)/np.sqrt(recDim);
	...
	def trainStep(self, instr, target):
		...
		return np.argmax(self.o), self.h.squeeze(), np.linalg.norm(self.dHH);
