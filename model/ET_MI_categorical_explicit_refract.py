...
class RNN:
	def __init__(self, inputDim, recDim, outDim):
		...
		self.dIH = np.zeros_like(self.IH);
		self.dHH = np.zeros_like(self.HH);
		self.dHO = np.zeros_like(self.HO);
	...
	def trainStep(self, instr, target):
		...
		# update HO
		self.dHO += np.outer(er, new_states);
		...
		self.dHH += dh*soft_step_prime*(self.eHH_volt - self.gamma*self.eHH_adapt) - self.mi*(hebbianHH - antiHebbianHH);
		self.dIH += dh*soft_step_prime*(self.eIH_volt - self.gamma*self.eIH_adapt) - self.mi*(hebbianIH - antiHebbianIH);
...
#		# sample with logistic distribution = diff of gumbel RV
	...
	def update(self):
		...
		self.HO -= self.rHO*self.dHO + self.lmbda*self.HO;
		self.HH -= self.rHH*self.dHH + self.lmbda*self.HH;
		self.IH -= self.rIH*self.dIH + self.lmbda*self.IH;
